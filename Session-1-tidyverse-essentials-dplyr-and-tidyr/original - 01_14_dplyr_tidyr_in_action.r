...
facturas_sustitucion_producto <- facturas_etl %>%
  select(
    producto,producto_no_homologado,
    subproducto,subproducto_no_homologado,
    grupo,grupo_no_homologado
  ) %>%
  filter(
    !is.na(producto),
    !is.na(producto_no_homologado)
  )
...
def process_data(data):
    ...
    for item in valid_data:
        item["normalized_value"] = (item["value"] - 10) / (100 - 10)

    # Generar estadísticas
    total = sum(item["normalized_value"] for item in valid_data)
    average = total / len(valid_data)
    min_value = min(item["normalized_value"] for item in valid_data)
    max_value = max(item["normalized_value"] for item in valid_data)
...
def filter_valid_data(data):
    """Filtrar datos no válidos."""
    return [item for item in data if item["value"] > 0 and item["status"] == "active"]

def normalize_values(data):
    """Normalizar valores en el rango 10-100."""
    for item in data:
        item["normalized_value"] = (item["value"] - 10) / (100 - 10)
    return data
...
def calculate_statistics(data):
    ...
    total = sum(item["normalized_value"] for item in data)
    average = total / len(data)
    min_value = min(item["normalized_value"] for item in data)
    max_value = max(item["normalized_value"] for item in data)
    return {"total": total, "average": average, "min": min_value, "max": max_value}

def display_results(stats):
    """Imprimir los resultados en pantalla."""
    print("Resultados del análisis:")
    print(f"Total: {stats['total']}")
    print(f"Promedio: {stats['average']}")
...
def process_data(data):
    ...
    normalized_data = normalize_values(valid_data)
